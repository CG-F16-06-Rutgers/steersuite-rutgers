...
*/
	...
	// He Chen
        // let the agents go to this place first
        SteerLib::AgentGoalInfo _temp, _prev;
        _prev = _goalQueue.front();
        _goalQueue.pop();
        _temp.targetLocation.y = 0;
	_temp.targetLocation.x = 36;
        if (_position.z <-30) {
                _temp.targetLocation.z = -34;
        }
        else if (_position.z < -10) {
                _temp.targetLocation.z = -13.5;
        }
	else if (_position.z < 10) {
		_temp.targetLocation.z = 1;
	}
	else if (_position.z < 30) {
		_temp.targetLocation.z = 18;
	}
	else {
		_temp.targetLocation.z = 35;
	}

        _goalQueue.push(_temp);
	_temp.targetLocation.x = 37.5;
	_goalQueue.push(_temp);
        _goalQueue.push(_prev);
        std::cerr<<"size of _goalQueue is: " << _goalQueue.size() << std::endl;
