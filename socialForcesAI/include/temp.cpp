...
*/
	...
	_deadlockWeight = static_cast<float> (rand() / static_cast<float> (RAND_MAX));
...
Util::Vector SocialForcesAgent::calcAgentRepulsionForce(float dt)
{
	...
	// for (int a =0; a < tmp_agents.size(); a++)
	{
		...
		{
		std::cerr<<"have a collision!!!!!!!!!!!!!!"<<std::endl;
...
Util::Point SocialForcesAgent::wallFollower(float dt)
{
        // He Chen
        // the real goal will never be reached, so the agent will keep following the wall
        std::set<SteerLib::SpatialDatabaseItemPtr> _neighbors;
                getSimulationEngine()->getSpatialDatabase()->getItemsInRange(_neighbors,
                                _position.x-(this->_radius + 150),
                                _position.x+(this->_radius + 150),
                                _position.z-(this->_radius + 150),
                                _position.z+(this->_radius + 150),
                                dynamic_cast<SteerLib::SpatialDatabaseItemPtr>(this));

        SteerLib::ObstacleInterface *tmp_obs;
        std::pair<float, Util::Point> min_stuff (1000, Util::Point(0,0,0)); // 1000 should be large enough
        Util::Vector min_wall_normal (0,0,0);
        std::pair<Util::Point, Util::Point> walline(Util::Point(0,0,0), Util::Point(0,0,0));
        // update _position to use wall_normal, need to come back to original _position
        _position = _position + _velocity * WALL_TIME_INTERVAL * dt;
        SteerLib::AgentGoalInfo tmp_add;
        tmp_add.targetLocation = _position;
        _goalQueue.push(tmp_add);
        for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbor = _neighbors.begin(); neighbor != _neighbors.end(); neighbor++) {
                if (!((*neighbor)->isAgent())) {
                        tmp_obs = dynamic_cast<SteerLib::ObstacleInterface *> (*neighbor);
                        Util::Vector wall_normal = calcWallNormal(tmp_obs);
                        std::pair<Util::Point, Util::Point> line = calcWallPointsFromNormal(tmp_obs, wall_normal);
                        std::pair<float, Util::Point> stuff = minimum_distance(line.first, line.second, position());
                        if (stuff.first < min_stuff.first) {
                                min_stuff.first = stuff.first;
                                min_stuff.second = stuff.second;
                                min_wall_normal = wall_normal;
                                walline = line;
                        }
                }
        }
        // now min_stuff contains the point on the wall which is nearest to _position
        // update the goal target position
        Util::Vector direc = position() - min_stuff.second;
        direc = direc/direc.length();
        direc = direc * WALL_OFFSET;
        _position = _position - _velocity * WALL_TIME_INTERVAL * dt;
        if ((min_stuff.second+direc-_position).length() < (_velocity*WALL_TIME_INTERVAL*dt).length())
                return min_stuff.second + 1.1*direc;
                //return min_stuff.second + 2*direc;
        return min_stuff.second + direc;
        //return min_stuff.second + min_wall_normal * WALL_OFFSET;
        // He Chen
}
...
// #define _DEBUG_ 1
	...
	_velocity = (prefForce) + repulsionForce/(1 + _deadlockWeight*10) + proximityForce/(1 + _deadlockWeight*10);
	// _velocity = (prefForce);
